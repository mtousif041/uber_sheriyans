...
module.exports.authCaptain = async (req, res, next)=>{
    // dhekiye token aapko do jahga milta hai header ke ander aur cookies ke ander , hum dono hi jagaha chek krne wale hai  ek ek krke 
    const token = req.cookies.token || req.headers.authorization?.split(' ')[1];
    ...
    try {
        ...
        const decoded = jwt.verify(token, process.env.JWT_SECRET);
        const captain = await captainModel.findById(decoded._id);//captainModel ke ander jab token create kr rhe te to data me sirf id hi dhai thi, to sirf utna hi data yaani id jab aap token ko decoded kroge to vo milega , yaani decoded se sirf id hi milegi knyki create krte time humne id hi di thi
